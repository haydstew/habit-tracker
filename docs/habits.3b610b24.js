let t,e;var n,s,o,i,a,r,l,c,d,u,h,f,m,g,p,E,C,y,O,v,_,I,w,b,T=globalThis,N={},A={},R=T.parcelRequire94c2;null==R&&((R=function(t){if(t in N)return N[t].exports;if(t in A){var e=A[t];delete A[t];var n={id:t,exports:{}};return N[t]=n,e.call(n.exports,n,n.exports),n.exports}var s=Error("Cannot find module '"+t+"'");throw s.code="MODULE_NOT_FOUND",s}).register=function(t,e){A[t]=e},T.parcelRequire94c2=R),R.register;var S=R("47Mwn"),M=R("ilpIi");(n=m||(m={})).STRING="string",n.NUMBER="number",n.INTEGER="integer",n.BOOLEAN="boolean",n.ARRAY="array",n.OBJECT="object",(s=g||(g={})).LANGUAGE_UNSPECIFIED="language_unspecified",s.PYTHON="python",(o=p||(p={})).OUTCOME_UNSPECIFIED="outcome_unspecified",o.OUTCOME_OK="outcome_ok",o.OUTCOME_FAILED="outcome_failed",o.OUTCOME_DEADLINE_EXCEEDED="outcome_deadline_exceeded";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function ta(t,e,n,s){return(await G(e,b.COUNT_TOKENS,t,!1,JSON.stringify(n),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function tr(t,e,n,s){return(await G(e,b.EMBED_CONTENT,t,!1,JSON.stringify(n),s)).json()}async function tl(t,e,n,s){let o=n.requests.map(t=>Object.assign(Object.assign({},t),{model:e}));return(await G(e,b.BATCH_EMBED_CONTENTS,t,!1,JSON.stringify({requests:o}),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class td{constructor(t){this.apiKey=t}getGenerativeModel(t,e){if(!t.model)throw new D("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new tc(this.apiKey,t,e)}getGenerativeModelFromCachedContent(t,e,n){if(!t.name)throw new k("Cached content must contain a `name` field.");if(!t.model)throw new k("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==e?void 0:e[n])&&t[n]&&(null==e?void 0:e[n])!==t[n]){if("model"===n&&(e.model.startsWith("models/")?e.model.replace("models/",""):e.model)===(t.model.startsWith("models/")?t.model.replace("models/",""):t.model))continue;throw new k(`Different value for "${n}" specified in modelParams (${e[n]}) and cachedContent (${t[n]})`)}let s=Object.assign(Object.assign({},e),{model:t.model,tools:t.tools,toolConfig:t.toolConfig,systemInstruction:t.systemInstruction,cachedContent:t});return new tc(this.apiKey,s,n)}}const tu=document.getElementById("habitInput"),th=document.getElementById("addHabitBtn"),tf=document.getElementById("habitList"),tm=document.getElementById("send-btn"),tg=document.getElementById("chat-input"),tp=document.getElementById("chat-history");let tE=!1;const tC=document.getElementById("signOutBtn");async function ty(){e=new td((await (0,M.getDoc)((0,M.doc)(S.db,"apikey","googlegenai"))).data().key).getGenerativeModel({model:"gemini-1.5-flash"})}function tO(t){tE||(tp.innerHTML="",tE=!0);let e=document.createElement("div");e.textContent=t,e.className="history",tp.appendChild(e),tp.scrollTop=tp.scrollHeight,tg.value=""}async function tv(t){let e=await tI(t);tu.value="",tT(e,t)}async function t_(){let t=await tw();tf.innerHTML="";let e=[];t.forEach(t=>{e.push({id:t.id,text:t.data().text,completed:t.data().completed})}),e.sort((t,e)=>new Date(e.timeCreated)-new Date(t.timeCreated)),e.forEach(t=>{tT(t.id,t.text,t.completed)})}async function tI(e){if(!t){alert("Error: No user email found. Please sign in again.");return}return(await (0,M.addDoc)((0,M.collection)(S.db,"habits"),{text:e,email:t,completed:!1})).id}async function tw(){let e=(0,M.query)((0,M.collection)(S.db,"habits"),(0,M.where)("email","==",t));return await (0,M.getDocs)(e)}async function tb(t){await (0,M.deleteDoc)((0,M.doc)(S.db,"habits",t))}function tT(t,e,n=!1){let s=document.createElement("li");s.id=t,s.classList.add("habit-item");let o=document.createElement("span");o.textContent=e,o.className=n?"completed":"habit-text";let i=document.createElement("div");i.classList.add("habit-buttons");let a=document.createElement("button");a.textContent=n?"Mark as Incomplete":"Mark as Complete",a.className="complete-btn",a.addEventListener("click",async()=>{n?window.confirm("Are you sure you want to mark this habit as incomplete?")&&(await tN(t,!n),t_()):window.confirm("Are you sure you want to mark this habit as complete?")&&(await tN(t,!n),t_())});let r=document.createElement("button");r.textContent="Delete Habit",r.className="delete-btn",r.addEventListener("click",async()=>{window.confirm("Are you sure you want to delete this habit?")&&(await tb(t),t_())}),i.appendChild(a),i.appendChild(r),s.appendChild(o),s.appendChild(i),tf.appendChild(s)}async function tN(t,e){await (0,M.updateDoc)((0,M.doc)(S.db,"habits",t),{completed:e})}async function tA(t){tO((await e.generateContent(t)).response.text())}window.addEventListener("load",async()=>{ty(),(0,S.auth).onAuthStateChanged(e=>{(t=e?e.email:localStorage.getItem("email"))?t_():window.location.href="index.html"})}),tm.addEventListener("click",async()=>{let t=tg.value.trim().toLowerCase();t?(t.includes("add")?tO('To add a new habit, enter a description into the text input under the "Add a New Habit" section, and then click on the "Add Habit" button.'):t.includes("delete")?tO('To delete a habit, click on its corresponding "Delete Habit" button under the "Your Habits" section.'):t.includes("modify")?tO('To modify the status of a habit, click on its corresponding "Mark as Complete/Incomplete" button under the "Your Habits" section.'):tO("Please specify if you would like to add, delete, or modify the status of a habit."),0):tO("Please enter a prompt.")}),tg.addEventListener("keypress",function(t){"Enter"===t.key&&tm.click()}),th.addEventListener("click",async()=>{let t=tu.value.trim();t?await tv(t):alert("Please enter a habit.")}),tu.addEventListener("keypress",function(t){"Enter"===t.key&&th.click()}),tC.addEventListener("click",async function(){window.confirm("Are you sure you want to sign out?")&&(await (0,S.auth).signOut(),localStorage.removeItem("authenticatedUser"),localStorage.removeItem("email"),window.location.href="index.html")});
//# sourceMappingURL=habits.3b610b24.js.map
