 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class x extends Error{constructor(e){super(`[GoogleGenerativeAI Error]: ${e}`)}}class D extends x{constructor(e,t){super(e),this.response=t}}class L extends x{constructor(e,t,n,s){super(e),this.status=t,this.statusText=n,this.errorDetails=s}}class H extends x{}(h=w||(w={})).GENERATE_CONTENT="generateContent",h.STREAM_GENERATE_CONTENT="streamGenerateContent",h.COUNT_TOKENS="countTokens",h.EMBED_CONTENT="embedContent",h.BATCH_EMBED_CONTENTS="batchEmbedContents";class U{constructor(e,t,n,s,o){this.model=e,this.task=t,this.apiKey=n,this.stream=s,this.requestOptions=o}toString(){var e,t;let n=(null===(e=this.requestOptions)||void 0===e?void 0:e.apiVersion)||"v1beta",s=(null===(t=this.requestOptions)||void 0===t?void 0:t.baseUrl)||"https://generativelanguage.googleapis.com",o=`${s}/${n}/${this.model}:${this.task}`;return this.stream&&(o+="?alt=sse"),o}}async function k(e){var t;let n=new Headers;n.append("Content-Type","application/json"),n.append("x-goog-api-client",function(e){let t=[];return(null==e?void 0:e.apiClient)&&t.push(e.apiClient),t.push("genai-js/0.21.0"),t.join(" ")}(e.requestOptions)),n.append("x-goog-api-key",e.apiKey);let s=null===(t=e.requestOptions)||void 0===t?void 0:t.customHeaders;if(s){if(!(s instanceof Headers))try{s=new Headers(s)}catch(e){throw new H(`unable to convert customHeaders value ${JSON.stringify(s)} to Headers: ${e.message}`)}for(let[e,t]of s.entries()){if("x-goog-api-key"===e)throw new H(`Cannot set reserved header name ${e}`);if("x-goog-api-client"===e)throw new H(`Header name ${e} can only be set using the apiClient field`);n.append(e,t)}}return n}async function P(e,t,n,s,o,i){let a=new U(e,t,n,s,i);return{url:a.toString(),fetchOptions:Object.assign(Object.assign({},function(e){let t={};if((null==e?void 0:e.signal)!==void 0||(null==e?void 0:e.timeout)>=0){let n=new AbortController;(null==e?void 0:e.timeout)>=0&&setTimeout(()=>n.abort(),e.timeout),(null==e?void 0:e.signal)&&e.signal.addEventListener("abort",()=>{n.abort()}),t.signal=n.signal}return t}(i)),{method:"POST",headers:await k(a),body:o})}}async function F(e,t,n,s,o,i={},a=fetch){let{url:r,fetchOptions:l}=await P(e,t,n,s,o,i);return G(r,l,a)}async function G(e,t,n=fetch){let s;try{s=await n(e,t)}catch(t){!function(e,t){let n=e;throw e instanceof L||e instanceof H||((n=new x(`Error fetching from ${t.toString()}: ${e.message}`)).stack=e.stack),n}(t,e)}return s.ok||await $(s,e),s}async function $(e,t){let n,s="";try{let t=await e.json();s=t.error.message,t.error.details&&(s+=` ${JSON.stringify(t.error.details)}`,n=t.error.details)}catch(e){}throw new L(`Error fetching from ${t.toString()}: [${e.status} ${e.statusText}] ${s}`,e.status,e.statusText,n)}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ec{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new x("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new el(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new H("Cached content must contain a `name` field.");if(!e.model)throw new H("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new H(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let s=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new el(this.apiKey,s,n)}}const ed=document.getElementById("habitInput"),eu=document.getElementById("addHabitBtn"),eh=document.getElementById("habitList"),ef=document.getElementById("send-btn"),em=document.getElementById("chat-input"),ep=document.getElementById("chat-history");let eg=!1;const eE=document.getElementById("signOutBtn");async function eC(){e=new ec((await (0,S.getDoc)((0,S.doc)(R.db,"apikey","googlegenai"))).data().key).getGenerativeModel({model:"gemini-1.5-flash"})}function ey(e){eg||(ep.innerHTML="",eg=!0);let t=document.createElement("div");t.textContent=e,t.className="history",ep.appendChild(t),ep.scrollTop=ep.scrollHeight,em.value=""}async function eO(e){let t=await e_(e);ed.value="",eb(t,e)}async function ev(){let e=await eI();eh.innerHTML="";let t=[];e.forEach(e=>{t.push({id:e.id,text:e.data().text,completed:e.data().completed})}),t.sort((e,t)=>new Date(t.timeCreated)-new Date(e.timeCreated)),t.forEach(e=>{eb(e.id,e.text,e.completed)})}async function e_(e){let t=R.auth.currentUser.email;return(await (0,S.addDoc)((0,S.collection)(R.db,"habits"),{text:e,email:t,completed:!1})).id}async function eI(){let e=R.auth.currentUser.email,t=(0,S.query)((0,S.collection)(R.db,"habits"),(0,S.where)("email","==",e));return await (0,S.getDocs)(t)}async function ew(e){await (0,S.deleteDoc)((0,S.doc)(R.db,"habits",e))}function eb(e,t,n=!1){let s=document.createElement("li");s.id=e,s.classList.add("habit-item");let o=document.createElement("span");o.textContent=t,o.className=n?"completed":"habit-text";let i=document.createElement("div");i.classList.add("habit-buttons");let a=document.createElement("button");a.textContent=n?"Mark as Incomplete":"Mark as Complete",a.className="complete-btn",a.addEventListener("click",async()=>{n?window.confirm("Are you sure you want to mark this habit as incomplete?")&&(await eT(e,!n),ev()):window.confirm("Are you sure you want to mark this habit as complete?")&&(await eT(e,!n),ev())});let r=document.createElement("button");r.textContent="Delete Habit",r.className="delete-btn",r.addEventListener("click",async()=>{window.confirm("Are you sure you want to delete this habit?")&&(await ew(e),ev())}),i.appendChild(a),i.appendChild(r),s.appendChild(o),s.appendChild(i),eh.appendChild(s)}async function eT(e,t){await (0,S.updateDoc)((0,S.doc)(R.db,"habits",e),{completed:t})}async function eN(t){ey((await e.generateContent(t)).response.text())}window.addEventListener("load",async()=>{eC(),ev()}),ef.addEventListener("click",async()=>{let e=em.value.trim().toLowerCase();e?(e.includes("add")?ey('To add a new habit, enter a description into the text input under the "Add a New Habit" section, and then click on the "Add Habit" button.'):e.includes("delete")?ey('To delete a habit, click on its corresponding "Delete Habit" button under the "Your Habits" section.'):e.includes("modify")?ey('To modify the status of a habit, click on its corresponding "Mark as Complete/Incomplete" button under the "Your Habits" section.'):ey("Please specify if you would like to add, delete, or modify the status of a habit."),0):ey("Please enter a prompt.")}),em.addEventListener("keypress",function(e){"Enter"===e.key&&ef.click()}),eu.addEventListener("click",async()=>{let e=ed.value.trim();e?await eO(e):alert("Please enter a habit.")}),ed.addEventListener("keypress",function(e){"Enter"===e.key&&eu.click()}),window.addEventListener("error",function(e){console.error("Error occurred: ",e.message)}),eE.addEventListener("click",async function(){window.confirm("Are you sure you want to sign out?")&&(localStorage.removeItem("authenticatedUser"),localStorage.removeItem("email"),window.location.href="index.html")});
//# sourceMappingURL=habits.a29dfbfc.js.map
