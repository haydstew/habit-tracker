 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function ta(t,e,n,s){return(await G(e,I.COUNT_TOKENS,t,!1,JSON.stringify(n),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function tr(t,e,n,s){return(await G(e,I.EMBED_CONTENT,t,!1,JSON.stringify(n),s)).json()}async function tl(t,e,n,s){let o=n.requests.map(t=>Object.assign(Object.assign({},t),{model:e}));return(await G(e,I.BATCH_EMBED_CONTENTS,t,!1,JSON.stringify({requests:o}),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class td{constructor(t){this.apiKey=t}getGenerativeModel(t,e){if(!t.model)throw new D("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new tc(this.apiKey,t,e)}getGenerativeModelFromCachedContent(t,e,n){if(!t.name)throw new U("Cached content must contain a `name` field.");if(!t.model)throw new U("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==e?void 0:e[n])&&t[n]&&(null==e?void 0:e[n])!==t[n]){if("model"===n&&(e.model.startsWith("models/")?e.model.replace("models/",""):e.model)===(t.model.startsWith("models/")?t.model.replace("models/",""):t.model))continue;throw new U(`Different value for "${n}" specified in modelParams (${e[n]}) and cachedContent (${t[n]})`)}let s=Object.assign(Object.assign({},e),{model:t.model,tools:t.tools,toolConfig:t.toolConfig,systemInstruction:t.systemInstruction,cachedContent:t});return new tc(this.apiKey,s,n)}}const tu=document.getElementById("habitInput"),th=document.getElementById("addHabitBtn"),tf=document.getElementById("habitList"),tm=document.getElementById("send-btn"),tg=document.getElementById("chat-input"),tp=document.getElementById("chat-history");let tE=!1;const tC=document.getElementById("signOutBtn"),ty=JSON.parse(localStorage.getItem("email"));async function tO(){return new Promise(t=>{(0,M.onAuthStateChanged)(S.auth,async e=>{e?(JSON.parse(localStorage.getItem("authenticatedUser"))||(alert("Biometric authentication required."),t(!1)),t(!0)):(alert("You must sign in first."),t(!1))})})}async function tv(){t=new td((await (0,R.getDoc)((0,R.doc)(S.db,"apikey","googlegenai"))).data().key).getGenerativeModel({model:"gemini-1.5-flash"})}function t_(t){tE||(tp.innerHTML="",tE=!0);let e=document.createElement("div");e.textContent=t,e.className="history",tp.appendChild(e),tp.scrollTop=tp.scrollHeight,tg.value=""}async function tw(t){if(!await tO())return;let e=await tb(t);tu.value="",tA(e,t)}async function tI(){let t=await tT();tf.innerHTML="";let e=[];t.forEach(t=>{e.push({id:t.id,text:t.data().text,completed:t.data().completed})}),e.sort((t,e)=>new Date(e.timeCreated)-new Date(t.timeCreated)),e.forEach(t=>{tA(t.id,t.text,t.completed)})}async function tb(t){let e=S.auth.currentUser;if(!e){alert("User not authenticated.");return}return(await (0,R.addDoc)((0,R.collection)(S.db,"habits"),{text:t,email:e.email,completed:!1})).id}async function tT(){let t=(0,R.query)((0,R.collection)(S.db,"todos"),(0,R.where)("email","==",ty));return await (0,R.getDocs)(t)}async function tN(t){await (0,R.deleteDoc)((0,R.doc)(S.db,"habits",t))}function tA(t,e,n=!1){let s=document.createElement("li");s.id=t,s.classList.add("habit-item");let o=document.createElement("span");o.textContent=e,o.className=n?"completed":"habit-text";let i=document.createElement("div");i.classList.add("habit-buttons");let a=document.createElement("button");a.textContent=n?"Mark as Incomplete":"Mark as Complete",a.className="complete-btn",a.addEventListener("click",async()=>{n?window.confirm("Are you sure you want to mark this habit as incomplete?")&&(await tS(t,!n),tI()):window.confirm("Are you sure you want to mark this habit as complete?")&&(await tS(t,!n),tI())});let r=document.createElement("button");r.textContent="Delete Habit",r.className="delete-btn",r.addEventListener("click",async()=>{window.confirm("Are you sure you want to delete this habit?")&&(await tN(t),tI())}),i.appendChild(a),i.appendChild(r),s.appendChild(o),s.appendChild(i),tf.appendChild(s)}async function tS(t,e){await (0,R.updateDoc)((0,R.doc)(S.db,"habits",t),{completed:e})}async function tR(e){t_((await t.generateContent(e)).response.text())}ty||(window.location.href="index.html"),window.addEventListener("load",async()=>{tv(),tI()}),tm.addEventListener("click",async()=>{let t=tg.value.trim().toLowerCase();t?(t.includes("add")?t_('To add a new habit, enter a description into the text input under the "Add a New Habit" section, and then click on the "Add Habit" button.'):t.includes("delete")?t_('To delete a habit, click on its corresponding "Delete Habit" button under the "Your Habits" section.'):t.includes("modify")?t_('To modify the status of a habit, click on its corresponding "Mark as Complete/Incomplete" button under the "Your Habits" section.'):t_("Please specify if you would like to add, delete, or modify the status of a habit."),0):t_("Please enter a prompt.")}),tg.addEventListener("keypress",function(t){"Enter"===t.key&&tm.click()}),th.addEventListener("click",async()=>{let t=tu.value.trim();t?await tw(t):alert("Please enter a habit.")}),tu.addEventListener("keypress",function(t){"Enter"===t.key&&th.click()}),window.addEventListener("error",function(t){console.error("Error occurred: ",t.message)}),tC.addEventListener("click",async function(){try{await (0,M.signOut)(S.auth),localStorage.removeItem("authenticatedUser"),window.location.href="index.html"}catch(t){console.error("Error signing out:",t)}});
//# sourceMappingURL=habits.5cb3b50a.js.map
